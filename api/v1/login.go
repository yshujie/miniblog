...
func Register(c *gin.Context) {
	...
	if code == errmsg.SUCCESS {
		...
		content := "<a href = 'http://localhost" + utils.HttpPort + "/api/v1/active?code=" + user.Code + "'>点我激活</a>"
...
func Login(c *gin.Context) {
	...
	if code == errmsg.SUCCESS {
		...
		if codeGetProfile == errmsg.SUCCESS {
			...
			//result, err:= model.Redis.Do("SET",token, profileJson).Result()
			if err != nil {
				fmt.Println("redis set fail", err)
			}
			//fmt.Println("set Profile :",result)
		}
	}
